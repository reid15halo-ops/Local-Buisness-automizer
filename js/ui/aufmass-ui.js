...
class AufmassUI {
    ...
    _svgRectangle(room, W, H, pad) {
        ...
        return `
            <rect x="${rx}" y="${ry}" width="${rw}" height="${rh}" fill="rgba(45,212,168,0.1)" stroke="#c8956c" stroke-width="2" rx="2" />
            ${deductions}
            <!-- Dimension labels -->
    ...
    _svgLForm(room, W, H, pad) {
        ...
        return `
            <polygon points="${points}" fill="rgba(45,212,168,0.1)" stroke="#c8956c" stroke-width="2" />
    ...
    _svgCircle(room, W, H, pad) {
        ...
        return `
            <circle cx="${cx}" cy="${cy}" r="${drawR}" fill="rgba(45,212,168,0.1)" stroke="#c8956c" stroke-width="2" />
    ...
    executeQuoteTransfer(projectId) {
        ...
        if (isNewAngebot) {
            ...
            const angebot = {
                ...
                mwst: Math.round(netto * ((typeof window._getTaxRate === 'function') ? window._getTaxRate() : 0.19) * 100) / 100,
                brutto: Math.round(netto * (1 + ((typeof window._getTaxRate === 'function') ? window._getTaxRate() : 0.19)) * 100) / 100,
                status: 'entwurf',
            ...
            angebot.mwst = Math.round(netto * ((typeof window._getTaxRate === 'function') ? window._getTaxRate() : 0.19) * 100) / 100;
            angebot.brutto = Math.round(netto * (1 + ((typeof window._getTaxRate === 'function') ? window._getTaxRate() : 0.19)) * 100) / 100;

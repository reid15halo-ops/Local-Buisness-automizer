...
class TrashService {
    constructor() {
        ...
        try { this.trash = JSON.parse(localStorage.getItem(this.TRASH_KEY) || '[]'); } catch { this.trash = []; }
    }
    ...
    getCurrentUser() {
        try {
            let profile; try { profile = JSON.parse(localStorage.getItem('freyai_company_profile') || '{}'); } catch { profile = {}; }

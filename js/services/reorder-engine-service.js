...
class ReorderEngineService {
    ...
    async checkAndReorder() {
        ...
        if (window.purchaseOrderService) {
            for (const [supplierName, items] of Object.entries(bySupplier)) {
                const po = await this._createPOForSupplier(supplierName, items);
                if (po) {
                    createdPOs.push(po);
                }
            }
     ...
     * @private
     */
    async _createPOForSupplier(supplierName, materials) {
        try {
            ...
            // Create the PO
            const po = await window.purchaseOrderService.createPO(supplier, positionen, {
    ...
    _logReorderActivity(auditEntry) {
        let log = StorageUtils.getJSON('reorder_activity_log', [], { service: 'reorderEngineService' });
        log.push(auditEntry);
    ...
    _logPOCreated(po, materials) {
        let log = StorageUtils.getJSON('reorder_po_log', [], { service: 'reorderEngineService' });
        log.push({
     ...
     */
    getPOLog() {
        return StorageUtils.getJSON('reorder_po_log', [], { service: 'reorderEngineService' }).reverse();
    }
